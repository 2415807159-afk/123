...
title_zh: 迁移学习结合嵌入式相关波函数理论实现化学精度的分子模拟：在碳酸钙离子配对中的应用
authors: "Xuezhi Bian, Emily A. Carter"
date: 2026-03-16
source: arxiv
link: "https://arxiv.org/abs/2603.15846v1"
tags: ["other:TiMD-ML"]
score: 8.712
...
motivation: 旨在解决凝聚态分子动力学模拟中难以兼顾化学精度与计算效率的挑战。
method: 提出ECW-TL框架，通过迁移学习将嵌入式相关波函数（如MP2、CCSD(T)）的高精度数据引入机器学习势能模型。
result: 在碳酸钙离子配对模拟中，该方法准确复现了自由能表面，并揭示了精确电子交换和相关效应对离子对稳定性的关键作用。
conclusion: ECW-TL为将高精度波函数理论应用于大规模复杂化学系统模拟提供了一种数据高效且通用的解决方案。
---

## 摘要
实现分子模拟的化学精度仍然是计算化学中的一个核心挑战。在此，我们提出了一种嵌入式相关波函数迁移学习（ECW-TL）框架，用于精确模拟凝聚相中的分子动力学。ECW-TL 在 ECW 理论中纳入了高能级电子交换和相关效应，同时保留了机器学习原子间势能的训练和计算效率。我们在水溶液中的 Ca2+-CO32- 离子配对上演示了该框架，这是海水二氧化碳矿化的一个关键基础过程。作为原理证明，我们首先展示了利用嵌入式 DFT-SCAN 数据微调 DFT-revPBE-D3(BJ) 基准模型，可以在所有溶剂化状态下重现误差在 1 kcal/mol 以内的 DFT-SCAN 自由能面。将该框架扩展到嵌入式 MP2 和局域自然轨道 CCSD(T) 进一步优化了自由能曲线，揭示了精确电子交换和相关在决定离子对稳定性和结构中的关键作用。因此，ECW-TL 为将相关波函数（CW）精度转移到复杂水溶液和界面化学过程的大规模模拟中提供了一种通用且数据高效的途径。
