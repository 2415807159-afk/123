...
## 摘要
化学语言模型 (CLMs) 已成为分子性质预测 (MPP) 任务中，传统机器学习模型的有力竞争者。然而，越来越多的研究报告称，CLMs 在各种 MPP 基准任务中的表现存在不一致且矛盾的结果。在本研究中，我们进行并分析了数百次精心控制的实验，以系统地研究各种因素（如数据集大小、模型大小和标准化）对 CLMs 在 MPP 预训练和微调性能方面的影响。在缺乏针对仅编码器掩码语言模型的成熟缩放法则（scaling laws）的情况下，我们的目标是提供全面的数值证据，并深入理解影响 CLMs 在 MPP 任务中性能的潜在机制，其中一些机制在现有文献中似乎被完全忽视了。
