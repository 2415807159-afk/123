...
tldr: PiGRAND是一个物理信息引导的图神经扩散框架，旨在解决增材制造中热传输模拟的高计算成本和数据稀缺问题。该方法结合了显式欧拉和隐式Crank-Nicolson数值方法，通过高效的图构建和迁移学习，实现了对热扩散过程的精确建模。实验表明，PiGRAND在预测精度和计算性能上均优于传统的GRAND和PINN模型，为智能制造提供了强有力的工具。
selection_source: fresh_fetch
motivation: 针对3D打印中热传输模拟计算复杂、传感器数据获取成本高且难以直接测量的问题。
method: 提出PiGRAND框架，将偏微分方程的物理原理融入图神经扩散模型，并结合高效图构建与迁移学习技术。
result: 在3D打印热图像数据集上的评估显示，PiGRAND在预测准确性和计算效率方面显著优于GRAND和PINNs。
conclusion: 通过将物理规律整合进图学习模型，PiGRAND为智能增材制造中的复杂热物理过程提供了一种高效且准确的建模方案。
---
