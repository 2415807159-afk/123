...
date: 2026-03-18
source: arxiv
link: "https://arxiv.org/abs/2603.17633v1"
tags: ["other:TiMD-ML"]
score: 8.37
...
## 摘要
理解生物分子的动态行为对于阐明生物功能和促进药物发现至关重要。虽然分子动力学（MD）模拟为研究这些动力学过程提供了严谨的物理基础，但在长时标上的计算成本依然高昂。相反，近期的深度生成模型虽然加速了构象生成，但通常要么无法建模时间关系，要么仅针对单体蛋白构建。为了弥补这一差距，我们引入了 ATMOS，这是一种基于状态空间模型（SSM）的新型生成框架，旨在为生物分子系统生成原子级 MD 轨迹。ATMOS 集成了一个基于 Pairformer 的状态转移机制以捕捉长程时间依赖性，并结合一个基于扩散的模块以自回归方式解码轨迹帧。ATMOS 在来自 PDB 的晶体结构以及包括 mdCATH 和 MISATO 在内的大规模 MD 模拟数据集的构象轨迹上进行了训练。我们证明了 ATMOS 在为蛋白质单体和复杂的蛋白质-配体系统生成构象轨迹方面均达到了最先进的性能。通过实现原子运动轨迹的高效推理，本研究为生物分子动力学建模奠定了坚实的基础。
