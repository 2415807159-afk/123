...
title_zh: sbml4md：一个由机器学习驱动的、通过分子动力学进行系统-浴模型建模的计算平台
authors: "Kwanghee Park, Seiji Ueno, Yoshitaka Tanimura"
date: 2026-03-18
source: arxiv
link: "https://arxiv.org/abs/2603.18274v1"
tags: ["other:TiMD-ML"]
score: 9.139
...
## 摘要
我们介绍了 sbml4md，这是一种新开发的算法，以软件包的形式实现，用于从分子动力学 (MD) 轨迹中提取多模非谐布朗 (MAB) 模型的参数，以模拟分子液体分子内模式的非线性振动光谱。通过利用机器学习 (ML) 技术来捕捉每种模式的振动非谐性、分子间耦合和浴相关函数，sbml4md 避免了经验拟合，并能够对具有空间和时间异质性的环境进行建模。这项工作提供了一组专门为分层运动方程 (HEOM) 框架定制的参数，从而实现了非线性振动光谱的数值“精确”模拟。在之前针对分子内振动模式的实现 [Park, Jo, and Tanimura, J. Chem. Phys. 163, 214104 (2025)] 的基础上，本代码通过显式考虑分子间振动贡献，提高了优化效率。这一扩展使 sbml4md 能够通过无缝集成经典 MD 方法，扩大基于 HEOM 的动力学建模的适用性，从而为在现实条件下以最少的经验输入模拟线性及非线性光谱提供了一个灵活且可扩展的框架。随附的 Python 编写的 ML 代码作为支持材料提供。
