...
## 速览
**TLDR**：远程外延技术通过在衬底和薄膜间插入二维材料（如石墨烯）来降低缺陷，但其物理机制尚不明确。本研究利用第一性原理计算，评估了多种预测指标，发现传统的静电势分布不足以解释该现象。研究提出“小岛滑动能垒”是判断远程外延可行性的最严谨指标，揭示了该过程本质上受石墨烯表面原子簇迁移的动力学控制。 \
**Motivation**：旨在寻找一种可靠的第一性原理指标，以预测特定衬底-薄膜组合是否适用于远程外延生长。 \
**Method**：采用第一性原理计算，对比分析了静电势分布及允许原子位置优化后的小岛滑动能垒等多种机制。 \
**Result**：发现静电势指标解释力不足，而优化后的小岛滑动能垒能最准确地判定远程外延的活性。 \
**Conclusion**：远程外延的成功与否主要取决于石墨烯表面小岛迁移的动力学难易程度。
