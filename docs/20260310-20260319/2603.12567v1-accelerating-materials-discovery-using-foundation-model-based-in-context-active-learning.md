...
date: 2026-03-13
source: arxiv
link: "https://arxiv.org/abs/2603.12567v1"
tags: ["other:TiMD-ML"]
score: 9.012
...
tldr: 本研究针对材料发现中主动学习代理模型（如GP和RF）在小样本量下拟合能力差或不确定性估计不准的问题，提出了基于基础模型的上下文主动学习（ICAL）。该方法采用在海量合成任务上预训练的TabPFN模型，通过单次前向传播实现高效的贝叶斯推理。实验表明，ICAL在多个材料数据集上显著优于传统方法，大幅减少了实验成本，证明了基础模型在加速材料研发中的巨大潜力。
selection_source: fresh_fetch
motivation: 现有的主动学习代理模型在处理样本量少于500的材料数据集时，难以兼顾复杂的成分-性能拟合与准确的不确定性量化。
method: 引入基于Transformer的TabPFN基础模型作为代理模型，利用其预训练的通用先验知识在无需重新训练的情况下进行上下文学习。
result: "在涵盖硬度、导电率等10个材料数据集的测试中，TabPFN在8个任务中获胜，比GP平均节省52%的实验次数。"
conclusion: 预训练基础模型凭借卓越的不确定性校准能力，成为加速主动学习驱动的材料发现的高效替代方案。
---
