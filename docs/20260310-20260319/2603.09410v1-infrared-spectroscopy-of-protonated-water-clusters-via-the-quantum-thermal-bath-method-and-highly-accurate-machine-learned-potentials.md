...
## 速览
**TLDR**：本研究旨在通过数值模拟准确捕捉质子化水团簇的红外光谱特征。研究结合了高精度的机器学习势能面（PES）和偶极矩面（DMS），并利用量子热浴（QTB）方法在分子动力学模拟中低成本地引入核量子效应（NQEs）。通过对从单体到四聚体的质子化水团簇进行模拟，证明了该方法在保持计算精度的同时，显著降低了计算成本，为研究水溶液局部环境提供了高效的理论工具。 \
**Motivation**：准确模拟水团簇的红外光谱通常需要昂贵的高级电子结构方法和量子动力学技术，亟需一种更高效的替代方案。 \
**Method**：结合高精度机器学习势能面与偶极矩面，并采用量子热浴（QTB）方法在分子动力学中纳入核量子效应。 \
**Result**：成功模拟了从单体到四聚体的质子化水团簇红外光谱，其结果与现有的理论和实验数据高度吻合。 \
**Conclusion**：机器学习势能面与量子热浴方法的结合，为研究复杂水系统的光谱性质提供了一种兼顾精度与效率的新途径。
