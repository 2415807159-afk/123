...
tldr: 本研究针对暗物质和中微子探测中核反冲电离产额的不确定性，提出了一种基于分子动力学模拟的新方法。该方法通过显式引入晶体凝聚态效应，克服了传统Lindhard模型的局限性，实现了从高能到单电子-空穴对能级的可靠预测。研究在硅探测器中取得了与实验数据的高度一致，并揭示了电离产额的分布特性，显著提升了低质量暗物质的探测灵敏度。
selection_source: fresh_fetch
motivation: 解决半导体探测器在极低能量下核反冲电离产额预测不准确及传统模型失效的问题。
method: 采用非参数化的分子动力学模拟方法，显式考虑晶体结构和凝聚态效应对离子输运的影响。
result: 在硅探测器上实现了与实验数据的高度吻合，并发现电离产额呈现分布特性而非单一数值。
conclusion: 该模型扩展了低质量暗物质的探测极限，并为高纯锗探测器的量子效应和沟道效应研究提供了新进展。
---
