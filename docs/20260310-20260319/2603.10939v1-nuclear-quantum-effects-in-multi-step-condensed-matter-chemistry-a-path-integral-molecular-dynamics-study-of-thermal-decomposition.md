...
date: 2026-03-11
source: arxiv
link: "https://arxiv.org/abs/2603.10939v1"
tags: ["other:TiMD-ML"]
score: 9.012
evidence: 快速模式：该论文与检索主题“molecular dynamics studies of titanium graphene composites and interfaces”相关，基于召回与重排结果优先保留。
tldr: "本研究探讨了核量子效应（NQEs）在凝聚态多步化学反应中的作用，以含能材料TATB的热分解为例。通过对比路径积分分子动力学（PIMD）、量子热浴（QTB）和经典分子动力学（ClMD），发现PIMD能更准确地捕捉氢转移过程中的量子加速效应，使活化能降低约8%，而QTB则显著高估了这种效应，为理解复杂凝聚态化学提供了重要参考。"
selection_source: fresh_fetch
motivation: 旨在阐明核量子效应在分子晶体多步、多分子化学反应中的具体影响，填补凝聚态研究中常忽略此类效应的空白。
method: 采用路径积分分子动力学（PIMD）模拟TATB晶体的热分解过程，并与量子热浴（QTB）和经典分子动力学（ClMD）的结果进行对比分析。
result: "PIMD模拟显示由于氢转移过程的量子效应，TATB分解速度加快且总活化能降低了约8%，而QTB显著高估了量子加速效果。"
conclusion: 核量子效应对凝聚态化学反应速率有显著影响，且在模拟此类复杂多步反应时，PIMD比QTB表现出更高的物理准确性。
---
