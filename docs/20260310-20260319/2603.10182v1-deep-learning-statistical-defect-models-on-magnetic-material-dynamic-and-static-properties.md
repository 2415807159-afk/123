...
## 速览
**TLDR**：本研究针对磁性材料中缺陷（如空位）对动态和静态特性的影响，提出了一种结合伪谱Landau-Lifshitz描述与深度学习的统计缺陷模型。通过开发卷积神经网络（CNN）和物理信息神经网络（PINN），研究实现了对色散关系和畴壁宽度的准确预测。该方法不仅能处理复杂的物理约束，还能在物理单位下进行统计预测，为发现新材料及确定实现特定拓扑结构所需的最小缺陷阈值提供了重要工具。 \
**Motivation**：现实磁性材料建模必须考虑缺陷影响，但传统方法难以高效关联缺陷阈值与物理观测值。 \
**Method**：结合伪谱Landau-Lifshitz描述，利用CNN、PINN及泛函连接理论构建统计模型，预测色散关系和畴壁宽度。 \
**Result**：成功开发了双分支CNN和物理信息方法，能够根据缺陷参数准确预测磁性材料的动态响应和静态结构。 \
**Conclusion**：该深度学习统计模型为材料发现和优化磁性器件性能提供了有效的物理预测手段。
