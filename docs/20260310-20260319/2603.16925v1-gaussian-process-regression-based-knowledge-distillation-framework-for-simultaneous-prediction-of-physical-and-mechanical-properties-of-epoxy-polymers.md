...
tldr: 针对环氧聚合物复杂结构和实验数据稀缺导致的机器学习预测难题，本研究提出了一种基于高斯过程回归知识蒸馏（GPR-KD）的框架。该框架利用GPR作为教师模型捕捉非线性关系，通过神经网络学生模型同时预测多种物理和力学性能。结合RDKit提取的分子描述符，该模型在处理多样化单体数据时表现出优异的准确性和泛化能力，为定制化环氧聚合物的加速设计提供了新途径。
selection_source: fresh_fetch
motivation: 环氧聚合物复杂的3D分子结构和实验数据集的匮乏限制了传统机器学习模型在多属性预测中的应用。
method: 提出一种GPR-KD框架，利用高斯过程回归作为教师模型提取特征关系，并由统一的神经网络学生模型通过知识蒸馏实现多属性同步预测。
result: 该框架在预测玻璃化转变温度、密度及多种力学性能方面优于传统模型，且通过跨属性相关性学习进一步提升了预测精度。
conclusion: 该研究证明了结合GPR鲁棒性与深度学习扩展性的知识蒸馏框架能有效加速具有特定性能的环氧聚合物设计。
---

## 摘要
环氧聚合物因其多功能特性而得到广泛应用，但由于其复杂的3D分子结构、多组分性质以及缺乏整理好的数据集，机器学习（ML）的应用仍然受限。现有的机器学习研究很大程度上局限于模拟数据、特定性能或狭窄的成分范围。为了解决这些局限性，我们开发了一种基于信息增强的高斯过程回归知识蒸馏（GPR-KD）框架，用于预测热固性环氧聚合物的多种物理性能（玻璃化转变温度、密度）和力学性能（弹性模量、拉伸强度、压缩强度、弯曲强度、断裂能、粘接强度）。该模型在涵盖多种单体类别（9种树脂，40种固化剂）的实验文献数据上进行了训练。独立的高斯过程回归模型作为教师模型，捕捉非线性的特征-性能关系，而统一的神经网络学生模型则同时学习所有性能的蒸馏知识。通过将目标性能编码为输入特征，学生模型利用了跨性能的相关性。使用RDKit从SMILES表示中提取的分子级描述符构建了一个物理信息模型。该框架结合了高斯过程回归的可解释性和鲁棒性，以及深度学习的可扩展性和泛化能力。对比分析表明，该模型的预测准确率优于传统的机器学习模型。通过相关性能之间的信息共享，同步多性能预测进一步提高了准确性。所提出的框架能够加速具有定制性能的新型环氧聚合物的设计。
