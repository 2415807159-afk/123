...
## 摘要
实现分子模拟的化学精度仍然是计算化学中的一个核心挑战。在此，我们提出了一种嵌入式相关波函数迁移学习（ECW-TL）框架，用于精确模拟凝聚相中的分子动力学。ECW-TL 在 ECW 理论中纳入了高级电子交换和相关效应，同时保留了机器学习原子间势的训练和计算效率。我们在水溶液中的 Ca2+-CO32- 离子配对上演示了该框架，这是海水二氧化碳矿化的关键基础过程。作为原理验证，我们首先展示了使用嵌入式 DFT-SCAN 数据微调 DFT-revPBE-D3(BJ) 基准模型，可以在所有溶剂化状态下重现 1 kcal/mol 以内的 DFT-SCAN 自由能面。将该框架扩展到嵌入式 MP2 和局域自然轨道 CCSD(T) 进一步优化了自由能剖面，揭示了精确电子交换和相关在确定离子对稳定性和结构中的关键作用。因此，ECW-TL 为将波函数（CW）精度转移到复杂水溶液和界面化学过程的大规模模拟中提供了一种通用且数据高效的途径。
