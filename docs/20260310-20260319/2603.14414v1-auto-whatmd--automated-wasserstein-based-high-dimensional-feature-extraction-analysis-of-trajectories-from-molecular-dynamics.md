...
title_zh: Auto-WHATMD：基于 Wasserstein 的分子动力学轨迹自动化高维特征提取分析
authors: "Sosuke Asano, Ikki Yasuda, Katsuhiro Endo, Yoshinori Hirano, Kenji Yasuoka"
date: 2026-03-15
source: arxiv
link: "https://arxiv.org/abs/2603.14414v1"
tags: ["other:TiMD-ML"]
score: 9.421
...
motivation: 传统的分子动力学轨迹分析依赖专家经验进行特征选择，存在主观性且难以处理高维时空数据。
method: 提出Auto-WHATMD算法，利用最优传输（Wasserstein）距离衡量轨迹差异，并通过模拟退火算法自动识别最具区分性的残基。
result: 在BRD4蛋白-配体系统应用中成功识别出环区关键残基，且少量选定残基即可捕捉与配体结合亲和力的相关性。
conclusion: 该方法能高效确定关键残基并为多类模拟系统设计特征，为定量比较复杂生物分子系统提供了自动化工具。
---

## 摘要
比较具有不同结合配体或突变等变化的多个蛋白质系统，并理解其影响，是分子动力学模拟的目标之一。通过少量特征来表征这些系统可以实现定量比较。然而，由于分子动力学模拟轨迹是高维时空数据，关键特征的选择依赖于领域专业知识，有时会引入主观假设。在这里，我们提出了一种利用最优传输距离（optimal transport distance）来比较高维轨迹数据的方法，并采用模拟退火算法来识别最能区分多个系统的残基。我们将该算法命名为 auto-WHATMD（基于 Wasserstein 的分子动力学轨迹自动化高维特征提取分析）。我们将 auto-WHATMD 应用于具有不同配体的溴结构域 4（bromodomain 4）的多个蛋白质-配体系统，识别出了环区（loop region）中最具辨别性的残基。此外，即使是少数选定的残基也足以捕捉与配体结合亲和力的相关性，这表明 auto-WHATMD 有效地优先筛选出了信息量最大的残基。我们的方法可用于高效确定关键残基，并为多个类似系统设计特征。
